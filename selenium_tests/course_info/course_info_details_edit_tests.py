...
class CourseInfoDetailsEditTests(CourseInfoBaseTestCase):
    ...
    # TODO: Both tests below are consistently failing for timeout exception,
    #  due to "get_input_field_value" method.  The method has a dependency on
    #  "self._wait_for_input_field_to_be_visible(field_name)".  If the
    #  field is not visible (or found), test cannot proceed to edit fields, etc
    #  The problem is with the new UI changes (Clicking on "Edit" toggles on
    # "Submit", the DOM doesn't get refreshed, so it never finds the editable
    #  fields for these tests (which looks for "input-course-{editable_field}")

    # A refresh won't work since you need to toggle the button again (same
    # state). Explict waits have also been added but to no avail.

    # Needs further cleanup and investigation.
    ...
    #
    #     '''DEBUG
    #     element = self.driver.find_element_by_id("input-course-description")
    #     print('displayed: {}, enabled: {}'.format(element.is_displayed(),
    #                                               element.is_enabled()))
    #     '''
    #
    #     self.driver.find_element_by_id("course-details-form-edit").click()
    #
    #     ''' DEBUG
    #     WebDriverWait(self.driver, 10).until(lambda s: s.find_element_by_id
    #         ("input-course-description").is_displayed())
    #     '''
    #
    #     original_field_values = {}
    #
