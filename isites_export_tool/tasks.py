...
def process_job(site_keyword):
    ...
    job.save()
    try :
        result = subprocess.check_output(["/usr/bin/ssh", settings.EXPORT_TOOL['ssh_hostname'], "-i", settings.EXPORT_TOOL['ssh_private_key'], settings.EXPORT_TOOL['create_site_zip_cmd'], "--keyword %s" % site_keyword],
...
def archive_jobs():
    ...
    for job in archivable_jobs:
        ...
        if (job.status == ISitesExportJob.STATUS_COMPLETE and job.output_file_name):
            try :
                result = subprocess.check_output(["/usr/bin/ssh", settings.EXPORT_TOOL['ssh_hostname'], "-i", settings.EXPORT_TOOL['ssh_private_key'], settings.EXPORT_TOOL['remove_site_zip_cmd'], "--filename %s" % job.output_file_name],
                                             stderr=subprocess.STDOUT)
            except subprocess.CalledProcessError as cpe:
                logger.error("SSH Process Error({0}: {1}".format(cpe.returncode, cpe.output))
                raise
